...
export class RemoteAttachPicker {
    ...
    public async ShowAttachEntries(config: any): Promise<string | undefined> {
        ...
        if (pipeTransport) {
            ...
            if (os.platform() === 'win32' &&
                ...
                !await debugUtils.checkFileExists(pipeTransport.pipeProgram)) {
                ...
                if (!await debugUtils.checkFileExists(config.pipeTransport.pipeProgram)) {
                ...
                if (!pipeProgram && config.pipeTransport.pipeCwd) {
                    ...
                    if (!await debugUtils.checkFileExists(newPipeProgramStr)) {
    ...
    private async getRemoteOSAndProcesses(pipeCmd: string, quoteArgs: boolean): Promise<AttachItem[]> {
        ...
        const output: string = await debugUtils.execChildProcess(execCommand, undefined, this._channel);
        ...
        if (lines.length === 0) {
            ...
            if (remoteOS !== "Linux" && remoteOS !== "Darwin") {
                throw new Error(vscode.l10n.t('Operating system "{0}" not supported.', remoteOS));
            }
    ...
    private async getRemoteProcessesExtendedRemote(miDebuggerPath: string, miDebuggerServerAddress: string): Promise<AttachItem[]> {
        ...
        let processListOutput: debugUtils.ProcessReturnType = await debugUtils.spawnChildProcess(miDebuggerPath, args);
        ...
        for (let i: number = 0; i < 5 && !processListOutput.succeeded && processListOutput.outputError.length === 0; i++) {
            processListOutput = await debugUtils.spawnChildProcess(miDebuggerPath, args);
        }
