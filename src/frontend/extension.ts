    ...
    fs.readdir(socketlists, (err, files) => {
        ...
        const pickedFile = (file: string) => {
            vscode.window.showInputBox({
                ...
                validateInput: (range: string) => {
                    return getMemoryRange(range) === undefined ? "Range must either be in format 0xF00-0xF01, 0xF100+32 or 0xABC154" : "";
                }
            }).then(range => {
                if (range) {
        ...
        if (files.length === 1) {
            ...
            vscode.window.showQuickPick(files, { placeHolder: "Running debugging instance" })
                .then(file => {
                    if (file) {
                        pickedFile(file);
                    }
